...
public class GPS_Service_ModeB extends Service implements LocationListener {
	...
	private StopTimerTask		stoptimerTask				= null;
	private RestartTimerTask	restarttimertask			= null;
	private int success_Flg	= 0;					// 測位成功フラグ
	...
	private AlarmManager alarmManager;					// アラームマネージャ
	private PendingIntent pi;
	private WakeLock wakelock;							// CPUロッククラス
	private boolean CPULockFlg				= false;	// CPU占有フラグ
	private boolean alarmManagerFLG		= false;	// アラームマネージャフラグ
	...
	class GPS_Service_ModeB_Binder extends Binder {
		GPS_Service_ModeB getService()
		{
			return GPS_Service_ModeB.this;
		}
	}

	public static final String Timer = "GPS timer B";
	...
	public void onStart(Intent intent, int startId) {
		...
		_PrefEditor.putString(getString(R.string.timer_state), "Re_start");
	...
	public void m_Start() {
		...
		if (gps_Flg == 1){ // GPSプロバイダが選択された ※めんどいから遅延時間はSleepで。直しはいずれ。
			...
			Log.d(getResources().getString(R.string.TAG), getResources().getString(R.string.PosStartApiLocationUpdates));
		}
		...
		if (timout_Flg == 0) {
			...
			if ((m_mtimes > 0) && (m_mtimes == measure_num)) {
				...
				if (rTimer == null) {
					...
					rTimer.schedule(restarttimertask, m_interval);
				}
			}
			...
			mHandler.post(new Runnable() {
				public void run() {
					if (success_Flg == 0) {
						...
						writeString[0] = "";	// 測位終了時刻
						writeString[1] = "0";	// 成功フラグ (1:成功、0:失敗)
						writeString[2] = "0";	// 緯度
						writeString[3] = "0";	// 経度
						writeString[4] = "0";	// 測位誤差
						writeString[5] = "";	// 測位時間1
						writeString[6] = "";	// 測位時間2
						writeString[7] = "";	// 測位種別
						...
						if ((m_mtimes > 0) && (m_mtimes == measure_num)) {
							m_Stop();
							_PrefEditor.putString(
									getString(R.string.timer_state),
									"timeout&num_over");
						}
						else {
							_PrefEditor.putString(
									getString(R.string.timer_state), "timeout");
							if (rTimer != null) {
								rTimer.cancel();
								rTimer = null;
							}
    ...
    public void save(String FileSort, String[] WriteDate) throws IOException {
		...
		if (!First_m_Flg) {
			First_m_Flg = true;
			++_defm_total_num;
			_PrefEditor.putString(getString(R.string.GPS_m_no), String.valueOf(_defm_total_num));
			_PrefEditor.commit();
		}
	...
	public IBinder onBind(Intent intent) {
		return new GPS_Service_ModeB_Binder();
	}
